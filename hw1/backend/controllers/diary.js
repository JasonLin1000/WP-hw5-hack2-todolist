import DiaryModel from "../models/diaryModels.js";
...
export const createDiary = async (req, res) => {
  const { description, date, tag, mood } = req.body;

  // Check description and other tags
  if (!description || !date || !tag || !mood) {
    return res
      .status(400)
      .json({ message: "Description, date, tag and mood are required!" });
  }
...
export const updateDiary = async (req, res) => {
  const { id } = req.params;
  const { description, tag, mood } = req.body;
